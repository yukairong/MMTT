...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, im2col_step=64):
        """
        多尺度注意力机制的可变形注意力模块
        :param d_model: 输入的embedding维度
        :param n_levels:    特征层数量
        :param n_heads: 注意力头数量
        :param n_points:    查询点个数
        :param im2col_step: 步长
        """
        super().__init__()
        ...
        # 用于将输入特征映射到值空间和将加权后的值映射回特征空间的线性层
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_padding_mask=None, query_attn_mask=None):
        ...
        # (N, Len_{query}, C) -> (N, Len_{query}, n_head * n_levels * n_point * 2) -> (N, Len_{query}, n_heads, n_levels, n_points, 2)
        sampling_offsets = self.sampling_offsets(query).view(N, Len_q, self.n_heads, self.n_levels, self.n_points, 2)
        # (N, Len_{query}, C) -> (N, Len_{query}, n_heads * n_levels * n_point) -> (N, Len_{query}, n_heads, n_levels * n_point)
        attention_weights = self.attention_weights(query).view(N, Len_q, self.n_heads, self.n_levels * self.n_points)
        # (N, Len_{query}, n_heads, n_levels * n_point) -> (N, Len_{query}, n_heads, n_levels * n_point) -> (N, Len_{query}, n_heads, n_levels, n_point)
        # -1代表只是对最后一个维度做softmax
        ...
        if reference_points.shape[-1] == 2:
            # 增加扩充一个heads维度和一个points维度
            # input_spatial_shapes也一样扩张为一致的维度
